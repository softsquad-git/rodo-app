        ...
        <div class="block-content block-content-full">
            ...
            <table v-if="data.data.length > 0 " class="table table-bordered table-striped table-vcenter">
                ...
                <tr>
                    ...
                    <th>Numer</th>
                    <th class="d-none d-sm-table-cell">Temat</th>
                ...
                <tr v-for="(item, index) in data.data">
                    ...
                    <td class="text-right">
                        <div class="btn-group">
                            <a :href="`/inspector/meetings/update/${item.id}`"
...
export default {
    ...
    methods: {
        ...
        loadItems(page = 1){
            this.$axios.get(this.list_url+`?page=${page}&id=${this.filters.id}&number=${this.filters.number}&title=${this.filters.title}&status_id=${this.filters.status_id}&pagination=${this.filters.pagination}`)
                .then((data) => {
                    this.data = data.data;
                })
        },
        remove(id) {
            this.$swal.fire({
                ...
                if (result.isConfirmed) {
                    this.$axios.delete(`/inspector/api/meetings/remove/${id}`)
                        .then((data) => {
