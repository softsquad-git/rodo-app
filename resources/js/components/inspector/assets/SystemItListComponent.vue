        ...
        <div class="block-content block-content-full">
            <div class="row mb-3">
                ...
                <div class="col-md-11 col-12">
                    <div class="row" v-if="isSearchBox">
                        ...
                        <div class="col-md-3">
                            <input type="text" v-model="filters.symbol" placeholder="Symbol" class="form-control form-control-sm form-control-alt">
                        </div>
                        <div class="col-md-3">
                            <input type="text" v-model="filters.name" placeholder="Nazwa" class="form-control form-control-sm form-control-alt">
                        </div>
                        <div class="col-md-3 col-12">
                            <input type="text" v-model="filters.owner" placeholder="Właściciel" class="form-control form-control-sm form-control-alt">
                        </div>
                        <div class="col-md-2">
            ...
            <table v-if="data.data.length > 0 " class="table table-bordered table-striped table-vcenter">
                ...
                <tr>
                    ...
                    <th class="d-none d-sm-table-cell" style="width: 30%;">Właściciel</th>
                ...
                <tr v-for="(item, index) in data.data">
                    ...
                    <td class="text-right">
                        <div class="btn-group">
                            <a :href="`/inspector/assets/system-it/update/${item.id}`"
...
export default {
    ...
    methods: {
        loadItems(page = 1) {
            this.$axios.get(this.list_url+`?page=${page}&pagination=${this.filters.pagination}&id=${this.filters.id}&symbol=${this.filters.symbol}&name=${this.filters.name}&status_id=${this.filters.status_id}&owner=${this.filters.owner}`)
                .then((data) => {
                    this.data = data.data;
                }).catch((error) => {
                //
        ...
        loadStatuses() {
            this.$axios.get(`/inspector/api/get-statuses?resource_type=system_it`)
                .then((data) => {
        ...
        remove(id) {
            this.$swal.fire({
                type: 'warning',
                title: 'Jesteś pewien?',
                text: 'Czy na pewno chcesz usunąć system?',
                ...
                if (result.isConfirmed) {
                    this.$axios.delete(`/inspector/api/assets/system-it/remove/${id}`)
                        .then((data) => {
