        ...
        <div class="block-content block-content-full">
            ...
            <table v-if="data.data.length > 0 " class="table table-bordered table-striped table-vcenter">
                ...
                <tr>
                    ...
                    <th class="d-none d-sm-table-cell" style="width: 30%;">Przydzielonych osób</th>
                ...
                <tr v-for="(item, index) in data.data">
                    ...
                    <td class="text-right">
                        <div class="btn-group">
                            <a :href="`/inspector/departments/update/${item.id}`"
...
export default {
    ...
    methods: {
        loadItems(page = 1) {
            this.$axios.get(this.list_url+`?page=${page}&pagination=${this.filters.pagination}&id=${this.filters.id}&number=${this.filters.number}&name=${this.filters.name}&status_id=${this.filters.status_id}`)
            .then((data) => {
                this.data = data.data;
            }).catch((error) => {
                //
        ...
        loadStatuses() {
            this.$axios.get(`/inspector/api/get-statuses?resource_type=department`)
                .then((data) => {
        ...
        remove(id) {
            this.$swal.fire({
                type: 'warning',
                title: 'Jesteś pewien?',
                text: 'Czy na pewno chcesz usunąć dział?',
                ...
                if (result.isConfirmed) {
                    this.$axios.delete(`/inspector/api/departments/remove/${id}`)
                        .then((data) => {
