        ...
        <div class="block-content block-content-full">
            ...
            <table v-if="data.data.length > 0 " class="table table-bordered table-striped table-vcenter">
                ...
                <tr>
                    ...
                    <th class="d-none d-sm-table-cell" style="width: 30%;">Opis</th>
                ...
                <tr v-for="(item, index) in data.data">
                    ...
                    <td class="fs-sm">
                        <ul>
                            <li v-for="test in item.tests">{{ test.name }}</li>
                        </ul>
                    </td>
                    <td class="fs-sm">
                        {{ item.status }}
                    ...
                    <td class="text-right">
                        <div class="btn-group">
                            <a :href="`/administration/certificates/patters/update/${item.id}`"
...
<script>
export default {
    name: "CertificatesPattersListComponent",
    data() {
        return {
            data: {},
            filters: {
    ...
    methods: {
        ...
        loadStatuses() {
            this.$axios.get(`/administration/api/get-statuses?resource_type=certificate_patter`)
                .then((data) => {
        ...
        remove(id) {
            this.$swal.fire({
                ...
                if (result.isConfirmed) {
                    this.$axios.delete(`/administration/api/certificates/patters/remove/${id}`)
                        .then((data) => {
